...
class CubieCube:
    def __init__(self, cube_input=None):
        ...
        (
            self.corner_permutation_dict,
            self.corner_orientation_dict,
            self.edge_permutation_dict,
            self.edge_orientation_dict,
        ) = Data.cubie_move_dicts

        def load_configuration_from(
            cube_input,
        ):  # Helper procedure to copy configuration from an input cubie_cube. (avoids repetition)
        ...
        if cube_input is not None:
            if isinstance(
                cube_input, FaceletCube
            ):  # Checks to see if a Facelet-Cube is input.
                load_configuration_from(cube_input.to_cubie_cube())
            elif isinstance(
                cube_input, CubieCube
            ):  # Checks to see if a Cubie-Cube is input.
    ...
    def move(self, move):
        ...
        turn_count = 1 + (
            move // 6
        )  # The procedure rotates a face clockwise only. Double and anti-clockwise rotation are achieved through repetition.
        face = move % 6
        ...
        for t in range(turn_count):
            ...
            for i in range(8):
                new_corner_permutations[i] = self.corner_permutations[
                    corner_permutation_map[i]
                ]
                new_corner_orientations[i] = self.corner_orientations[
                    corner_permutation_map[i]
                ]  # It is imperative to also permute the information regarding the orientation of the corners.
            ...
            # Apply map.
            for i in range(8):
                new_corner_orientations[i] = (
                    self.corner_orientations[i] + corner_orientation_map[i]
                ) % 3  # Note the use of modular arithmetic.
            ...
            for i in range(12):
                new_edge_permutations[i] = self.edge_permutations[
                    edge_permutation_map[i]
                ]
                new_edge_orientations[i] = self.edge_orientations[
                    edge_permutation_map[i]
                ]
            ...
            for i in range(12):
                new_edge_orientations[i] = (
                    self.edge_orientations[i] + edge_orientation_table[i]
                ) % 2
    ...
    def __repr__(self):
        return (
            f"Corner Permutations: {self.corner_permutations}\n"
    ...
    def verify_solvability(self):
        ...
        corner_permutation_parity = (
            sum(
                a > b
                for i, a in enumerate(self.corner_permutations)
                for b in self.corner_permutations[i + 1 :]
            )
            % 2
        )
        ...
        edge_permutation_parity = (
            sum(
                a > b
                for i, a in enumerate(self.edge_permutations)
                for b in self.edge_permutations[i + 1 :]
            )
            % 2
        )
        ...
        if corner_twist_valid and edge_flip_valid and parity_consistent:
            ...
            print(
                f"Error: The input cube is unsolvable."
    ...
    def scramble(self):
        ...
        corner_permutation_parity = (
            sum(
                a > b
                for i, a in enumerate(corner_permutations)
                for b in corner_permutations[i + 1 :]
            )
            % 2
        )
        # Check edge permutation parity
        edge_permutation_parity = (
            sum(
                a > b
                for i, a in enumerate(edge_permutations)
                for b in edge_permutations[i + 1 :]
            )
            % 2
        )
        # Parity consistency check
        ...
        if not parity_consistent:
            edge_permutations[0], edge_permutations[1] = (
                edge_permutations[1],
                edge_permutations[0],
            )

        corner_orientations = [random.randint(0, 2) for i in range(7)]
        corner_orientations.append(
            -sum(corner_orientations) % 3
        )  # force divisiblity by 3
...
class FaceletCube:
    def __init__(self, cube_input=None):
        ...
        self.corner_colour_table, self.edge_colour_table = (
            Data.corner_colour_table,
            Data.edge_colour_table,
        )  # These map colours to cubie pieces.
        self.corner_facelet_indices, self.edge_facelet_indices = (
            Data.corner_facelet_indices,
            Data.edge_facelet_indices,
        )  # These map pieces to their facelets. ie: UFL to U6, L2, F0.

        self.colours_on_face_map = (
            Data.colours_on_face_map
        )  # Used to cycle the colours on a face.
    ...
    def from_cubie_cube(self, cubie_cube):
        ...
        for i in range(8):
            ...
            for j in range(3):
                ...
                facelets[self.corner_facelet_indices[i][j]] = (
                    self.corner_colour_table[permutation]
                )[(j + orientation) % 3]
        ...
        for i in range(12):
            ...
            for j in range(2):
                facelets[self.edge_facelet_indices[i][j]] = self.edge_colour_table[
                    permutation
                ][(j + orientation) % 2]
    ...
    def to_cubie_cube(self):
        ...
        for i, corner_index in enumerate(self.corner_facelet_indices):
            ...
            orientation = (
                -(colours.index("W") if "W" in colours else colours.index("Y")) % 3
            )
        ...
        for i, edge_index in enumerate(self.edge_facelet_indices):
            ...
            orientation = next(
                colours.index(color) for color in orientation_colors if color in colours
            )
    ...
    def rotate_colours_on_face(self, move):
        ...
        face_indices = self.facelets[
            start_pointer : start_pointer + 9
        ]  # Isolate the face.
...
class CoordCube:
    def __init__(self, cube_input=None):
        ...
        def load_configuration_from(
            cube_input,
        ):  # Helper procedure to copy configuration from an input CoordCube
            self.corner_orientation_coordinate = (
                cube_input.corner_orientation_coordinate
            )
            ...
            self.eight_edge_permutation_coordinate = (
                cube_input.eight_edge_permutation_coordinate
            )
            self.four_edge_permutation_coordinate = (
                cube_input.four_edge_permutation_coordinate
            )
            self.corner_permutation_coordinate = (
                cube_input.corner_permutation_coordinate
            )
    ...
    def from_cubie_cube(self, cubie_cube):
        ...
        coord_cube.corner_orientation_coordinate = (
            coord_cube.calculate_corner_orientation_coordinate()
        )
        coord_cube.edge_orientation_coordinate = (
            coord_cube.calculate_edge_orientation_coordinate()
        )
        coord_cube.UD_slice_coordinate = coord_cube.calculate_UD_slice_coordinate()
        coord_cube.eight_edge_permutation_coordinate = (
            coord_cube.calculate_eight_edge_permutation_coordinate()
        )
        coord_cube.four_edge_permutation_coordinate = (
            coord_cube.calculate_four_edge_permutation_coordinate()
        )
        coord_cube.corner_permutation_coordinate = (
            coord_cube.calculate_corner_permutation_coordinate()
        )

        coord_cube.g1_coordinates = [
            coord_cube.corner_orientation_coordinate,
            coord_cube.edge_orientation_coordinate,
            coord_cube.UD_slice_coordinate,
        ]
        coord_cube.g2_coordinates = [
            coord_cube.eight_edge_permutation_coordinate,
            coord_cube.four_edge_permutation_coordinate,
            coord_cube.corner_permutation_coordinate,
        ]
    ...
    def rotate_clockwise(self, move):
        ...
        self.g1_coordinates = [
            (self.g1_move_tables[i])[str(self.g1_coordinates[i])][move]
            for i in range(3)
        ]
        self.g2_coordinates = [
            (self.g2_move_tables[i])[str(self.g2_coordinates[i])][move]
            for i in range(3)
        ]
    ...
    def calculate_UD_slice_coordinate(self):
        slice_indices = [
            1 if i in [8, 9, 10, 11] else 0 for i in self.cubie_cube.edge_permutations
        ]
    ...
    def calculate_corner_permutation_coordinate(self):
        ...
        for i in range(8):
            smaller_count = sum(
                1
                for j in range(i)
                if self.cubie_cube.corner_permutations[j]
                > self.cubie_cube.corner_permutations[i]
            )
    ...
    def calculate_eight_edge_permutation_coordinate(self):
        ...
        for i in range(8):
            smaller_count = sum(
                1
                for j in range(i)
                if self.cubie_cube.edge_permutations[j]
                > self.cubie_cube.edge_permutations[i]
            )
    ...
    def calculate_four_edge_permutation_coordinate(self):
        ...
        for i in range(4):
            smaller_count = sum(
                1
                for j in range(i)
                if self.cubie_cube.edge_permutations[j + 8]
                > self.cubie_cube.edge_permutations[i + 8]
            )
