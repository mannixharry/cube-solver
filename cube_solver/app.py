...
from . import capture, cube
from .data import *
from .engine import *
from .paths import RESOURCES_DIR
from .screen import Button, Renderer

class Main:
    def __display_program_info(self, renderer, information):
        '''Displays information on the screen and flips the display.'''
        renderer.display_info(information)
        pygame.display.flip()

    def __wait(self):
        '''Blocks until the user presses Enter (or quits with Escape).'''
        waiting = True
        while waiting:
    ...
    def __initialize_buttons(self, renderer):
        '''Loads button images and attaches the Button objects to renderer.'''
        width, height = renderer.width, renderer.height

        scramble_image = pygame.image.load(RESOURCES_DIR / 'scramble.png')
        reset_image = pygame.image.load(RESOURCES_DIR / 'reset.png')
        solve_image = pygame.image.load(RESOURCES_DIR / 'solve.png')
        capture_image = pygame.image.load(RESOURCES_DIR / 'capture.png')
        pause_play_image = pygame.image.load(RESOURCES_DIR / 'pause-play.png')
        replay_image = pygame.image.load(RESOURCES_DIR / 'replay.png')
        skip_image = pygame.image.load(RESOURCES_DIR / 'skip.png')
    ...
    def main(self, cube_string='WWWWWWWWWGGGGGGGGGOOOOOOOOORRRRRRRRRBBBBBBBBBYYYYYYYYY'):
        '''Main game loop: handles user input, drives the solve demonstration, and ties together
        engine.py (3D rendering), solver.py and capture.py.

        Args:
            cube_string (str, optional): initial cube state. Defaults to a solved cube.
        '''
        renderer = Renderer()
        ...
        move_interval_ms = 3000  # Time each demonstrated move is shown for.
        ...
        while running:
            ...
            clicked_button_name = clicked_button.name if clicked_button else None
            ...
            if is_demonstrating_solve:
                ...
                elif clicked_button_name == 'replay' and solution_index != 0:
            ...
            if is_demonstrating_solve and not is_paused:
                ...
                if current_time - last_move_time >= move_interval_ms:
                    ...
                    # Demonstration proceeds through 3 sub-stages per move: (0) optionally re-orient the
                    # view for an awkward face-pair transition, (1) turn to face the move, (2) perform it.
                    if solve_stage % 3 == 0:
                        if solution_index == 0:
                            solve_move = solution[solution_index]
                            setting_view = False
            ...
            # Manual face turns, from click or keyboard.
            ...
            if not is_demonstrating_solve:
                ...
                if clicked_facelet is not None:
...
def main():
    '''Application entry point, called by run.py.

    Imports are ordered carefully to avoid a circular import: move tables must exist on disk before
    cube.import_tables() runs, and pruning tables must exist before solver is imported (since
    importing solver eagerly loads cube's move tables and its own pruning tables).
    '''
    global solver

    from . import move_table_generator
    move_tables = move_table_generator.MoveTableGenerator()
    move_tables.move_table_generation()
    cube.import_tables()

    from . import pruning_table_generator, solver
